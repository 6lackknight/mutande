...
| Weekly note | `pilot/sessions/YYYY-MM-DD-weekly.md` | Waitlist + Mixpanel + asks; latest: `2026-08-20-weekly.md` |
...
- Waitlist: **9 entries** (Aug 4–8, 2026) — last successful `/admin/ops` export **2026-08-13**. 2026-08-20 weekly run: hub `/health` ok; admin waitlist/feedback **401** on placeholder Auth0 token (not Cloudflare 1010); no Mixpanel Query creds. PRs #7/#8/#9/#12/#14/#15 still unanswered
- Mixpanel: instrumented on web + desktop 1.1.4; **live counts missing** — iOS / `surface=mobile` not shipped. Desktop coverage ~7 days; published cut still **2.0.7**. Unreleased main (Aug 19) adds first-job cards + invite after handshake (`desktop_handoff_next` kinds). Try Alpha still goes through the waitlist form. Download picker is chooser-first (`platform_picker`). See weekly note ask
