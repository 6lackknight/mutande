...
| Weekly note | `pilot/sessions/YYYY-MM-DD-weekly.md` | Waitlist + Mixpanel + asks; latest: `2026-08-21-weekly.md` |
...
- Waitlist: **9 entries** (Aug 4–8, 2026) — last successful `/admin/ops` export **2026-08-13**. 2026-08-21 weekly run: hub `/health` ok; admin waitlist/feedback **401** on placeholder Auth0 token (not Cloudflare 1010); no Mixpanel Query creds. PRs #7/#8/#9/#12/#14/#15/#16 still unanswered
- Mixpanel: instrumented on web + desktop 1.1.4; **live counts missing** — iOS / `surface=mobile` not shipped. Desktop coverage ~8 days; published cut now **2.0.10** (onboarding + first-job cards + version-matched Windows installer). Try Alpha still goes through the waitlist form. Download picker is chooser-first (`platform_picker`). See weekly note ask
