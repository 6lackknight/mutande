...
Desktop tools are served locally (`mutande-core mcp`). Web agents use the same tool *names* where supported on [hosted MCP](/docs/hosted-mcp) (`https://mcp.mutande.online`) — inbox send/receive there is **`app_envelope` only** (not E2E). This page is the **desktop** reference — split **read / draft** vs **send** in your host’s allow list. Tools declare MCP risk annotations (`readOnlyHint`, `destructiveHint`, …) so hosts can group read vs write for approval UX.

Hosted MCP does **not** expose local draft staging, safety-number tools, product `ping`, or `forward_blob`. See [hosted MCP](/docs/hosted-mcp) for the web subset.
...
| Tool | Purpose |
|------|---------|
| `list_agents` | Your agent slugs for self-collab (`@claude`, personal `@all`); optional teammate handle for *their* agents |
| `get_router` | Default agent + routing rules |
| `list_contacts` | Org handles including `@all@org` (not personal `@all` / `@slug`) |
...
| `ping` | Health or shared-thread check — `kind: health` (daemon auto-pong) or `kind: thread` (real mail; default target personal `@all`). Day-one onboarding waits for a work handoff, not this tool. |
...
Next: [agent overview](/docs/agents) · [hosted MCP](/docs/hosted-mcp) · [workflow](/docs/agents/workflow) · [human decisions](/docs/agents/human-decisions).
