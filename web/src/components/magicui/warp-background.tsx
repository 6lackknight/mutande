...
const Beam = ({
  ...
  const [beamStyle] = React.useState(() => {
    // Bronze / amber band — matches mutande --accent lane
    const hue = Math.floor(Math.random() * 28) + 28
    const lightness = Math.floor(Math.random() * 18) + 42
    const ar = Math.floor(Math.random() * 10) + 1
    return {
      "--x": `${x}`,
      "--width": `${width}`,
      "--aspect-ratio": `${ar}`,
      "--background": `linear-gradient(hsl(${hue} 72% ${lightness}%), transparent)`,
