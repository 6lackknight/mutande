...
export function createCollabRoutes(store: HubStore) {
  ...
  routes.post("/:id/steerers", async (c) => {
    const body = await c.req.json<{ handle: string }>();
    const collab = await store.addCollabSteerer(
      c.get("auth"),
      c.req.param("id"),
      body,
    );
  ...
  routes.post("/:id/steerers/remove", async (c) => {
    const body = await c.req.json<{ user_id: string }>();
    const collab = await store.removeCollabSteerer(
      c.get("auth"),
      c.req.param("id"),
      body,
    );
