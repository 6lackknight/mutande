...
4. Recipients work the thread; use `reply_to_thread` (optional `to_agent` for self-handoff). Sender sees replies on the same thread — do not open a new one for each hop.
