...
- Onboarding is self-serve create-team or join-invite (Mac-first); org slug is user-picked; handle defaults to `email-local@org`. Post-onboard: blocking host connect, then first-run ping wizard (copy prompt, poll Threads, celebrate — complete when a pong is received; skipping allowed); progress in `~/.mutande/first_run.json`. Mac Contacts tab loads hub `list_contacts` — your-handle card, broadcast callout, copy/Message on teammates; solo org shows invite CTA to prod web `/admin/invites`.
...
- Mac app bundle id is `ai.mutande.app`; bundles `mutande-core` sidecar in app Resources and launches it on startup; v1 ships as notarized DMG via `scripts/release-macos-dmg.sh`; Silicon alpha is served from `/downloads/mutande-alpha.dmg` via the `/download` page (alpha channel, rolling latest only; artifacts gitignored under `web/public/downloads/`).
