...
- Prefer Developer ID + notarized DMG for Mac v1 distribution; bump app version with each alpha cut; ship a single rolling alpha artifact (`mutande-alpha.dmg`) — keep only the latest on the site; on rebuild/redeploy always ship Mac and Windows together (trigger Windows Actions first for wall-clock, run Mac notarized DMG locally in parallel); dedicated web `/download` page; landing nav Docs · Download · Sign in (Get started / Join waitlist only in the hero — don’t pair Sign in with Get started in nav); Download CTA labeled Try Alpha; DMG binaries under `web/public/downloads/` are gitignored (do not commit); defer Mac App Store until the product can live in sandbox (daemon sidecar + silent MCP config writes conflict today).
