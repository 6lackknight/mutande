...
- Large payloads use private R2 with object key prefix `blobs/{id}` (`R2_*` hub env). Public installers ship Mac + Windows together to `mutande-releases` at `https://downloads.mutande.online` (Mac: notarized DMG via `scripts/release-macos-dmg.sh` then `scripts/upload-downloads-r2.sh`; Windows: Actions `Release Windows alpha` / `publish-r2`; `R2_DOWNLOADS_*` — not hub blobs tokens). Mac app bundle id `ai.mutande.app` bundles `mutande-core` sidecar in Resources; site `/download` gates Intel/Windows via `NEXT_PUBLIC_*_PUBLISHED`; local `web/public/downloads/*` gitignored only.
